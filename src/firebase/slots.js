...
export const addSlot = async (schoolId, slotData) => {
  try {
    ...
    const newSlot = {
      ...
      volunteerCount: parseInt(slotData.totalSlots) || 1,
      capacity: 150,
      filledSlots: 0,
      volunteerNames: [],
      signedUpUsers: [],
...
export const updateSlot = async (schoolId, slotId, updates) => {
  try {
    ...
    // Process updates: if totalSlots is provided, map it to volunteerCount
    const processedUpdates = { ...updates };
    if (processedUpdates.totalSlots !== undefined) {
      processedUpdates.volunteerCount = parseInt(processedUpdates.totalSlots);
      delete processedUpdates.totalSlots;
    }
    // Always keep capacity at 150
    processedUpdates.capacity = 150;
...
export const signUpForSlot = async (schoolId, slotId, userName) => {
  try {
    ...
    const capacity = slotData.capacity || 150;
    const volunteerNames = slotData.volunteerNames || [];
    if (volunteerNames.length >= capacity) {
      throw new Error("Slot is full");
    }

    // Check if user is already signed up
    const alreadySignedUp = volunteerNames.some((name) => name === userName);
    if (alreadySignedUp) {
      throw new Error("User is already signed up for this slot");
    }

    // Add user to volunteerNames
    const updatedVolunteerNames = [...volunteerNames, userName];
...
export const cancelSlotSignup = async (schoolId, slotId, userName) => {
  try {
    ...
    const volunteerNames = slotData.volunteerNames || [];
    const userIndex = volunteerNames.indexOf(userName);
    if (userIndex === -1) {
      // Check if user is admin
      const userRef = doc(usersCollection, auth.currentUser.uid);
      const userSnap = await getDoc(userRef);
    ...
    const updatedVolunteerNames = volunteerNames.filter(
      (name) => name !== userName
    );
...
/**
 * Update the capacity of all slots for all schools to 150
 * @returns {Promise<void>}
 */
export const updateAllSlotsCapacity = async () => {
  try {
    const schools = await getDocs(collection(db, "schools"));
    ...
    for (const schoolDoc of schools.docs) {
      ...
      for (const slotDoc of slotsSnapshot.docs) {
        const slotRef = doc(db, "schools", schoolDoc.id, "slots", slotDoc.id);
        await updateDoc(slotRef, {
          capacity: 150,
        });
      }
    }
    console.log("All slots have been updated to a capacity of 150.");
  } catch (error) {
    console.error("Error updating all slots capacity:", error);
    throw error;
  }
};
