...
export const addUserToSlot = async (schoolId, slotId, userId, userName) => {
  console.log(
    "DEBUG: addUserToSlot called with schoolId:",
    schoolId,
    "slotId:",
    slotId,
    "userId:",
  ...
  try {
    ...
    console.log(
      "DEBUG: Current user authenticated, uid:",
    ...
    if (!userSnap.exists() || userSnap.data().role !== "capstoneAdmin") {
      console.log(
        "DEBUG: User is not admin, role:",
        userSnap.exists() ? userSnap.data().role : "user not found"
      );
    ...
    console.log("DEBUG: Error in addUserToSlot:", error);
