...
const SlotManager = ({ schoolId, schoolName, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingSlot) {
        ...
        // refresh from server to ensure stored format matches display
        const slotsData = await getSlots(schoolId);
        setSlots(slotsData);
        setShowEditModal(false);
      } else {
        console.log("DEBUG: Adding new slot");
        await addSlot(schoolId, slotData);
        // refresh from server to ensure stored format matches display
        const slotsData = await getSlots(schoolId);
        setSlots(slotsData);
  ...
  const handleAddUser = async () => {
    ...
    try {
      // Use signUpForSlot function instead
      await signUpForSlot(
        schoolId,
      ...
      const updatedSlots = slots.map((slot) =>
        slot.id === showManageSignups.id
          ? {
              ...slot,
              volunteerNames: [...slot.volunteerNames, user.name || user.email],
            }
          : slot
      );
  ...
  const handleRemoveUser = async (userName) => {
    console.log("DEBUG: Removing user from slot, userName:", userName);
    try {
      await cancelSlotSignup(schoolId, showManageSignups.id, userName);
      ...
      const updatedSlots = slots.map((slot) =>
        slot.id === showManageSignups.id
          ? {
              ...slot,
              volunteerNames: slot.volunteerNames.filter(
                (name) => name !== userName
              ),
            }
          : slot
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600">
                      ...
                      {slot.volunteerNames?.length || 0} / {slot.capacity}
                    </p>
                      ...
                      slot.volunteerNames?.length > 0 && (
                        <div className="mt-2">
                          ...
                          <ul className="list-disc list-inside text-sm text-gray-600">
                            {slot.volunteerNames.map((name, index) => (
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
              ...
              <div className="mb-4">
                ...
                {showManageSignups.volunteerNames?.length > 0 ? (
                  <ul className="space-y-2">
                    {showManageSignups.volunteerNames.map((name, index) => (
                      <li
                        key={index}
              ...
              <div className="mb-4">
                ...
                >
                  ...
                  {users
                    .filter(
                      (user) =>
                        !showManageSignups.volunteerNames?.some(
