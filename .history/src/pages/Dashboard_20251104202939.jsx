...
import { updateWebsiteContent } from "../firebase/websiteContent";
...
const Dashboard = () => {
  ...
  const handleResetWebsite = async () => {
    if (
      !window.confirm(
        "Are you sure you want to reset the website to its original state? This will delete all schools, sponsors, slots, events, donations, and user data. This action cannot be undone."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const schoolDeletePromises = schoolsSnapshot.docs.map((doc) =>
        deleteDoc(doc.ref)
      );
      ...
      const sponsorDeletePromises = sponsorsSnapshot.docs.map((doc) =>
        deleteDoc(doc.ref)
      );
      await Promise.all(sponsorDeletePromises);

      // Delete all events
      const eventsSnapshot = await getDocs(collection(db, "events"));
      const eventDeletePromises = eventsSnapshot.docs.map((doc) =>
        deleteDoc(doc.ref)
      );
      ...
      const donationDeletePromises = donationsSnapshot.docs.map((doc) =>
        deleteDoc(doc.ref)
      );
      await Promise.all(donationDeletePromises);
      ...
      toast.success("Website has been reset to its original state");
    } catch (error) {
      console.error("Error resetting website:", error);
      toast.error("Failed to reset website. Please try again.");
    } finally {
      setResetting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-secondary-50">
      <header className="bg-primary shadow-lg rounded-b-lg">
          ...
          <div className="flex justify-between items-center py-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="bg-primary hover:bg-accent text-white px-4 py-2 rounded-lg shadow-md text-sm font-medium transition-colors duration-200"
              >
                Logout
              </button>
              {currentUser?.email === "theknowledgenetwork2025@gmail.com" && (
                <button
                  ...
                  disabled={resetting}
                  className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg shadow-md text-sm font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 mt-8">
        <div className="px-4 py-6 sm:px-0">
          {!selectedSchool ? (
            <div className="border-4 border-dashed border-gray-200 rounded-lg shadow-sm h-96 flex items-center justify-center">
            ...
            <div className="border-4 border-dashed border-red-200 rounded-lg shadow-sm h-96 flex items-center justify-center">
              <div className="text-center">
