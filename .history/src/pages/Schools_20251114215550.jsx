          ...
          <div>
            ...
            {!currentUser && (
              <p className="text-red-600 mt-2 font-medium">
                You must login to sign up for tutoring slots.
              </p>
            )}
          </div>
          {canEdit && (
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-2">
                    {schoolSlots[school.id].map((slot) => {
                      ...
                      // compute signed-up count robustly
                      const signedUpCount = Array.isArray(slot.volunteerNames)
                        ? slot.volunteerNames.length
                        : Array.isArray(slot.signedUpUsers)
                        ? slot.signedUpUsers.length
                        : 0;

                      // The displayed "Volunteers Available" is the volunteerCount entered when creating/editing
                      const displayedVolunteerCount =
                        slot.volunteerCount || slot.totalSlots || 4;

                      // The actual capacity for signups is always 150
                      const actualCapacity = 150;

                      // Slot is full when actual signups reach 150
                      const isFull = signedUpCount >= actualCapacity;
                      ...
                      const available = displayedVolunteerCount;

                      if (
                        !Number.isFinite(displayedVolunteerCount) ||
                        Number.isNaN(available)
                      ) {
                        // eslint-disable-next-line no-console
                        console.warn(
                          "Schools: unexpected volunteerCount for slot",
                          slot.id,
                          {
                            volunteerCount: slot.volunteerCount,
                            totalSlots: slot.totalSlots,
                            displayedVolunteerCount,
                            signedUpCount,
                            slot,
                          }
                        );
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center space-x-6">
                                  ...
                                  <p className="text-sm text-gray-600">
                                    <strong>Signed Up:</strong> {signedUpCount}{" "}
                                    / 150
                                  </p>
                                </div>
                              </div>
            ...
            <form onSubmit={handleUpdateSlot}>
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600"
                  rows="3"
                  placeholder="Optional description for this tutoring slot"
                />
              </div>
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              {editingSlot &&
                editingSlot.volunteerNames &&
                editingSlot.volunteerNames.length > 0 && (
                  <div className="mb-4">
                    <h3 className="text-lg font-semibold mb-2">
                      Signed Up Volunteers
                    </h3>
                    <ul className="list-disc list-inside text-sm text-gray-700">
                      {editingSlot.volunteerNames.map((name, index) => (
                        <li key={index}>{name}</li>
                      ))}
                    </ul>
                  </div>
