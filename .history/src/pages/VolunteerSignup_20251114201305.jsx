              ...
              {schoolSlots[school.id] && schoolSlots[school.id].length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {schoolSlots[school.id].map((slot) => {
                    ...
                    const signedUpCount =
                      (Array.isArray(slot.signedUpUsers) && slot.signedUpUsers.length) ||
                      (Array.isArray(slot.volunteerNames) && slot.volunteerNames.length) ||
                      0;

                    const capacityNumber = (() => {
                      const candidates = [slot.capacity, slot.totalSlots, slot.capacityOverride];
                    ...
                    const isSignedUp = Array.isArray(slot.signedUpUsers)
                      ...
                      ? slot.volunteerNames.some((name) => name.includes(currentUser?.email || ""))
                      : false;

                    const isFull = signedUpCount >= capacityNumber;
                    const isAvailable = !isFull && !isSignedUp;
                    const available = Math.max(0, capacityNumber - signedUpCount);
