                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-2">
                    {schoolSlots[school.id].map((slot) => {
                      ...
                      // parse capacity safely (try several fields and formats)
                      const capacityNumber = (() => {
                        const candidates = [slot.capacity, slot.totalSlots];
                        for (const c of candidates) {
                          ...
                          if (typeof c === "string") {
                            ...
                            const asNum2 = Number(digits);
                            if (!Number.isNaN(asNum2) && asNum2 >= 0)
                              return asNum2;
                          }
                        }
                        return NaN;
                      ...
                      const isFull = Number.isFinite(capacityNumber)
                        ? signedUpCount >= capacityNumber
                        : volunteerNames.length >= (slot.capacity || 150);

                      const isAvailable = !isFull && !isSignedUp;
                      const available = Number.isFinite(capacityNumber)
                        ? Math.max(0, capacityNumber - signedUpCount)
                        : Math.max(
                            0,
                            (Number(slot.capacity) || 0) - signedUpCount
                          );

                      if (!Number.isFinite(capacityNumber) || Number.isNaN(available)) {
                        // eslint-disable-next-line no-console
                        console.warn("Schools: unexpected capacity for slot", slot.id, {
