              ...
              {schoolSlots[school.id] && schoolSlots[school.id].length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {schoolSlots[school.id].map((slot) => {
                    // Robustly determine signed-up count and capacity because
                    // different parts of the app may use different field names.
                    const signedUpCount =
                      (Array.isArray(slot.signedUpUsers) &&
                        slot.signedUpUsers.length) ||
                      (Array.isArray(slot.volunteerNames) &&
                    ...
                    const isSignedUp = Array.isArray(slot.signedUpUsers)
                      ? slot.signedUpUsers.some(
                          (user) => user.userId === currentUser?.uid
                        )
                      : Array.isArray(slot.volunteerNames)
                      ? slot.volunteerNames.some((name) =>
                          name.includes(currentUser?.email || "")
                        )
                      : false;

                    const isFull = signedUpCount >= capacityNumber;
                    const isAvailable = !isFull && !isSignedUp;
                    const available = Math.max(
                      0,
                      capacityNumber - signedUpCount
                    );

                    if (!Number.isFinite(capacityNumber) || Number.isNaN(available)) {
                      // eslint-disable-next-line no-console
                      console.warn("VolunteerSignup: unexpected capacity for slot", slot.id, {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-600 mb-3">
                          <strong>Volunteers Available:</strong> {available}
                        </p>
