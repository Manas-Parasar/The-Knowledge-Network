...
const AdminYearManagement = () => {
  ...
  const handleMigrateSlotCapacity = async () => {
    if (
      !window.confirm(
        "This will update all existing slots to have a maximum capacity of 150 volunteers. Continue?"
      )
    ) {
      return;
    }

    setMigratingCapacity(true);
    try {
      await updateAllSlotsCapacity();
      toast.success("All slots have been updated to a capacity of 150");
    } catch (err) {
      console.error("Error migrating slot capacity:", err);
      toast.error("Failed to migrate slot capacity. Please try again.");
    } finally {
      setMigratingCapacity(false);
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
          <button
            onClick={handleMigrateSlotCapacity}
            disabled={migratingCapacity}
            className="bg-orange-500 text-white px-4 py-2 rounded-lg shadow-sm hover:bg-orange-600 disabled:bg-gray-400 transition-colors"
          >
            {migratingCapacity ? "Migrating..." : "Migrate Slot Capacity to 150"}
          </button>
          <button
